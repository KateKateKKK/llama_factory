...
import subprocess
from src.llamafactory.extras.misc import get_device_count, is_env_enabled, use_ray
import random
import sys
...
def create_app(chat_model: "ChatModel") -> "FastAPI":
    ...
    @app.post(
        "/model_finetune",
        response_model=ModelFinetuneResponse,
        status_code=status.HTTP_200_OK,
        dependencies=[Depends(verify_api_key)],
    )
    async def start_model_finetune(request: FinetuneRequest):
        ...
        if force_torchrun or (get_device_count() > 1 and not use_ray()):
            ...
            model_args = {
                "output_dir": "D:\workspace\llama_factory\LLaMA-Factory-0.9.1\output",
        ...
        return ModelFinetuneResponse(success = True)
